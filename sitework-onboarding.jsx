  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "20px 0 0" }}>
      ...
      <p className="fade-up-2" style={{
        ...
        lineHeight: 1.65, marginBottom: 36, maxWidth: 280,
      }}>
        {hi
      ...
      <div className="fade-up-3" style={{
        display: "flex", gap: 8, marginBottom: 36, flexWrap: "wrap", justifyContent: "center",
      }}>
        {[
          { e: "🏢", l: hi ? "System Integrator" : "System Integrator" },
          { e: "🔧", l: hi ? "Technician" : "Technician" },
          { e: "🔄", l: hi ? "दोनों" : "Both" },
        ].map(r => (
          <div key={r.l} style={{
      ...
      <div className="fade-up-4" style={{ width: "100%" }}>
        <button onClick={onNext} style={{
          ...
          fontFamily: font.display, fontSize: 16, fontWeight: 700,
          color: T.ink, transition: "all 0.18s",
...
const StepSIProfile = ({ onNext, lang }) => {
  ...
  return (
    <div>
      ...
      <Btn onClick={onNext} disabled={!canProceed} style={{ background: T.sky, boxShadow: `0 4px 20px ${T.sky}50` }}>
        {hi ? "Profile Save करें — शुरू करें" : "Save Profile — Get Started"}
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
      </Btn>
    </div>
...
export default function OnboardingApp() {
  ...
  const totalDots = 5;
  const dotStep = Math.max(0, step - 1); // dots start from step 1
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ width: "100%", maxWidth: 420, padding: "20px 20px 40px", flex: 1 }}>
          ...
          {step === 0 && <StepLogin onNext={() => setStep(1)} lang={lang} />}
          {step === 1 && <StepTerms onNext={() => setStep(2)} lang={lang} />}
          {step === 2 && <StepMobile onNext={() => setStep(3)} lang={lang} />}
          {step === 3 && <StepRole onNext={handleRoleNext} lang={lang} />}
          {step === 4 && (role === "SI"
            ? <StepSIProfile onNext={() => setStep(5)} lang={lang} />
            : <StepTechProfile onNext={() => setStep(5)} lang={lang} />
          )}
          {step === 5 && <StepSuccess role={role} lang={lang} />}
        </div>
      </div>
    </>
  );
