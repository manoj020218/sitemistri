...
export default function SIDashboard() {
  ...
  // Read Web Share Target params from URL (set by Android share sheet)
  const _shareParams = (() => {
    const p = new URLSearchParams(window.location.search);
    return { shareUrl: p.get("url") || p.get("text") || "", geoParam: p.get("geo") || "" };
  })();

  const [screen,         setScreen]         = useState(() => {
    if (_shareParams.shareUrl || _shareParams.geoParam) return "search";
  ...
  // Clear share params from URL so refresh doesn't re-trigger
  useEffect(() => {
    if (_shareParams.shareUrl || _shareParams.geoParam) {
          ...
          <>
            ...
            <div style={{ display: screen === "search" ? "block" : "none" }}>
              <SearchScreen
                ...
                initialShareUrl={_shareParams.shareUrl}
                initialGeoParam={_shareParams.geoParam}
              />
            </div>
            ...
            {screen === "profile"      && <SIProfileScreen user={user} siProfile={siProfile} lang={lang} onProfileSaved={p=>setSiProfile(p)} />}
