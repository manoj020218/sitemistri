...
// ─── Search Form Screen ───────────────────────────────────────────
export const SearchScreen = ({ lang, setScreen, setSelectedTech, setSearchContext, initialShareUrl, initialGeoParam }) => {
  const hi = lang === "hi";
  ...
  // Auto-fill location when opened via Google Maps share or geo: URI
  const autoFilled = useRef(false);
  useEffect(() => {
    if (autoFilled.current) return;
    if (initialShareUrl && /maps\.app\.goo\.gl|maps\.google\.com|google\.com\/maps|goo\.gl\/maps/i.test(initialShareUrl)) {
      autoFilled.current = true;
      setLinkVal(initialShareUrl);
      handleResolveLink(initialShareUrl);
    } else if (initialGeoParam) {
      // geo:lat,lng — no API call needed, parse coords directly
      const raw = initialGeoParam.replace(/^geo:/i, "").split(",");
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`)
          .then(r => r.json())
          .then(d => setSiteAddr(d.display_name || `${lat.toFixed(5)}, ${lng.toFixed(5)}`))
          .catch(() => {});
      }
    }
  }, [initialShareUrl, initialGeoParam]); // eslint-disable-line react-hooks/exhaustive-deps
