...
const StepLogin = ({ onNext, lang }) => {
  ...
  const roles = [
    { id: "SI",   e: "🏢", l: hi ? "System Integrator" : "System Integrator",
      colors: { bg: T.skyDim, border: T.sky, text: T.sky } },
    { id: "TECH", e: "🔧", l: hi ? "Technician" : "Technician",
      colors: { bg: T.saffronDim, border: T.saffron, text: T.saffron } },
    { id: "BOTH", e: "🔄", l: hi ? "दोनों (SI + Tech)" : "Both (SI + Tech)",
      colors: { bg: T.greenDim, border: T.green, text: T.green } },
  ];
  ...
  const tryLogin = () => {
    if (!preRole) {
      setError(hi ? "पहले अपना role चुनें — Technician, SI, या दोनों" : "Please select your role first — Technician, SI, or Both");
      return;
    }
    handleGoogleLogin();
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "20px 0 0" }}>
      ...
      {/* Role selection — required before login */}
      <div className="fade-up-3" style={{ width: "100%", marginBottom: 24 }}>
        <div style={{
          fontFamily: font.mono, fontSize: 11, fontWeight: 700,
          letterSpacing: "0.1em", color: T.muted, textAlign: "center",
          marginBottom: 12, textTransform: "uppercase",
        }}>
          {hi ? "आप कौन हैं? (जरूरी)" : "Who are you? (Required)"}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {roles.map((r) => {
            const sel = preRole === r.id;
            return (
              <button key={r.id} onClick={() => setPreRole(sel ? null : r.id)} style={{
                width: "100%", padding: "14px 18px",
                borderRadius: 14, border: `2px solid ${sel ? r.colors.border : T.border}`,
                background: sel ? r.colors.bg : T.white,
                cursor: "pointer", display: "flex", alignItems: "center", gap: 14,
                transition: "all 0.2s",
                boxShadow: sel ? `0 3px 16px ${r.colors.border}30` : "none",
              }}>
                <div style={{
                  width: 40, height: 40, borderRadius: 12,
                  background: sel ? r.colors.border : T.paper,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 20, flexShrink: 0, transition: "background 0.2s",
                }}>{r.e}</div>
                <div style={{
                  fontFamily: font.display, fontSize: 15, fontWeight: sel ? 700 : 500,
                  color: sel ? r.colors.text : T.ink, flex: 1, textAlign: "left",
                }}>{r.l}</div>
                <div style={{
                  width: 20, height: 20, borderRadius: "50%",
                  border: `2px solid ${sel ? r.colors.border : T.border}`,
                  background: sel ? r.colors.border : T.white,
                  display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div className="fade-up-4" style={{ width: "100%" }}>
        <button onClick={tryLogin} disabled={loading} style={{
          width: "100%", padding: "15px 20px",
          borderRadius: 50, border: `2px solid ${preRole ? T.saffron : T.border}`,
          background: preRole ? T.saffron : T.white,
          cursor: loading ? "not-allowed" : "pointer",
          display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
          fontFamily: font.display, fontSize: 16, fontWeight: 700,
          color: preRole ? T.white : T.ink, transition: "all 0.2s",
          boxShadow: preRole ? "0 4px 20px rgba(232,99,10,0.35)" : "0 4px 16px rgba(0,0,0,0.08)",
          opacity: loading ? 0.7 : 1,
        }}>
          {loading ? (
            <div style={{ width: 20, height: 20, border: `2px solid ${preRole ? T.white : T.saffron}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
          ) : !preRole ? (
